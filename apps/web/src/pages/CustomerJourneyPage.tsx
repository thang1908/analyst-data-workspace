...
const CustomerJourneyPage: React.FC = () => {
  ...
  const loadJourney = useCallback(async () => {
    ...
    try {
      const [nextStages, nextSteps] = await Promise.all([
      ...
      setStages(nextStages); setSteps(nextSteps);
    } catch (loadError) {
  ...
  return <>
    ...
    <div className="page-content">
      ...
      {!loading && !error && <>
        ...
        {stages.length ? <div className="journey-stages animate-in" style={{ marginBottom: 24 }}>
        ...
        </div> : <AnalyticsState message="Không có lifecycle stage khớp bộ lọc." />}
